...
function isUpdateAction(value: unknown): value is UpdateAction {
  ...
  if (action.kind !== "update" || !Number.isInteger(action.foodId) || !action.changes || typeof action.changes !== "object") return false;
...
export function isAssistantAction(value: unknown): value is AssistantAction {
  ...
  if (action.kind === "batch") {
    return Array.isArray(action.actions) && action.actions.length > 0 && action.actions.every(isBatchItem) && new Set(action.actions.map((item) => item.foodId)).size === action.actions.length;
  }
  return false;
}
...
export function addBatchFoodNames(action: AssistantAction, foods: Food[]): AssistantAction {
  if (action.kind !== "batch") return action;
  return {
    ...action,
    actions: action.actions.map((item) => ({ ...item, foodName: foods.find((food) => food.id === item.foodId)?.name ?? "Selected item" })),
  };
}
