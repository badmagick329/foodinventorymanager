...
function getReasoningEffort(): ReasoningEffort | null {
  ...
  return reasoningEfforts.includes(effort as ReasoningEffort)
...
export async function GET(request: NextRequest) {
  ...
  if (!conversationId)
    return NextResponse.json({ messages: [], ...assistantConfiguration() });
  ...
  if (!conversation)
    return NextResponse.json(
      { messages: [], ...assistantConfiguration() },
      { status: 404 }
    );
  return NextResponse.json({
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!conversation)
      return NextResponse.json(
        { error: "That chat no longer exists. Start a new chat." },
        { status: 404 }
      );
    ...
    return NextResponse.json(
      {
        error:
          "The assistant chat could not be prepared. Restart the app and try again.",
      },
      { status: 500 }
    );
  }
  ...
  const stream = new ReadableStream({
    async start(controller) {
      sse(controller, encoder, "conversation", {
      ...
      try {
        ...
        const call = completedResponse?.output?.find(
          (item) =>
            item.type === "function_call" &&
            item.name === "propose_inventory_action"
        );
        ...
        const estimatedCostUsd = usage
          ? estimateAssistantCost(usage, model)
          : null;
